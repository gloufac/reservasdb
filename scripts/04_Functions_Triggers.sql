...
-- Obtener calendario de disponiblidad: no funciona bien, version 1.0
CREATE OR REPLACE FUNCTION public.get_availability_by_detalle_and_dates_OLD(p_detalleid int, p_fechainicio bigint, p_fechafin bigint)
...
BEGIN
	...
	SELECT 	main.fecha::varchar(20) as str_fecha
			...
			,pd.id as detalleid
			,CAST(pd.disponibles - coalesce(SUM(rd.cantidad), 0) AS INT) as disponibles
			,coalesce(SUM(rd.cantidad),0) as total
